...
async function editTunnel(editTunnel) {
    const response = await fetch(`http://localhost:3000/tunnels/${editTunnel.id}`,
        {
            method: "PATCH",
            headers: {
    ...
    if (response.ok) {
        tunnels.splice(tunnels.findIndex((_t) => _t.id === editTunnel.id), 1, await response.json());
        tunnelDialog.value = false;
        toast.add({ severity: 'success', summary: 'Succès', detail: `Tunnel ${editTunnel.commentaire} modifié avec succès`, life: 3000 });
    ...
    <div class="card">
        <div class="my-2">
            ...
            <DataTable ref="dt" :value="tunnels" v-model:selection="selectedTunnels" dataKey="id" :paginator="true"
                ...
                <Column field="updatedAt" header="Modifier le" headerStyle="width:20%; min-width:18rem;">
                    <template #body="slotProps">
            ...
            <Dialog v-model:visible="tunnelDialog" :style="{ width: '450px' }" header="Modifier le tunnel" :modal="true"
                ...
                <template #footer>
                    ...
                    <Button label="Valider" icon="pi pi-check" class="p-button-text" @click="editTunnel(tunnel)" />
                </template>
            </Dialog>
        </div>
    </div>
